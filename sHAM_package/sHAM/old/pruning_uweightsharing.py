...
from sHAM import uweightsharing
from sHAM import weightsharing
...
class PruninguWeightsharing_NN(pruning.Pruning_NN, uweightsharing.uWeightsharing_NN):
    ...
    def apply_pr_uws(self, list_trainable=None, untrainable_per_layers=None, mbkmeans=True):
        self.apply_pruning(list_trainable, untrainable_per_layers) #crea masks e setta pesi prunati nel modello
        self.apply_uws(list_trainable, untrainable_per_layers, mbkmeans) #crea centri e matrici degli indici e setta pesi ws nel modello
        gc.collect()
    ...
    def update_centers_and_recompose(self, list_weights_before, lr):
        ...
        centers_upd = [(centroid_gradient_matrix_combined(self.idx_layers[(i-d)//div], list_weights[i] - list_weights_before[i], self.clusters, masks_for_centers[(i-d)//div])) for i in range(d,len(list_weights), div)]
    ...
    def recompose_weight(self, list_weights, trainable_vars=False, untrainable_per_layers=None):
        ...
        if not trainable_vars:
            ...
            return list_weights[:d]+[(weightsharing.idx_matrix_to_matrix(self.idx_layers[(i-d)//self.div], self.centers)*masks_for_centers[(i-d)//self.div]) if i%self.div==0 else (list_weights[i]) for i in range(d,len(list_weights))]
        else:
            ...
            d = find_index_first_dense(list_weights)
            return list_weights[:d]+[(weightsharing.idx_matrix_to_matrix(self.idx_layers[(i-d)//div], self.centers)*masks_for_centers[(i-d)//div]) if i%div==0 else (list_weights[i]) for i in range(d,len(list_weights))]
